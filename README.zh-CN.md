...
- 可分别配置直链音频、网易云音乐和哔哩哔哩自动播放；三个选项均默认关闭。
- 网易云播放器和直链音频控件适配 Flarum 深色模式，并支持 FoF Night Mode 动态切换主题。
...
直链音频、网易云音乐和哔哩哔哩拥有各自独立的自动播放选项，且均默认关闭。即使启用了自动播放，浏览器仍可能在访客与网站交互前阻止有声播放。
